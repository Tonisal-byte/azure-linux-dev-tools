...
const (
	...
	// OriginTypeRust2RPM indicates that the source file is generated by rust2rpm.
	OriginTypeRust2RPM OriginType = "rust2rpm"
)
...
type Origin struct {
	...
	Type OriginType `toml:"type" json:"type" jsonschema:"required,enum=download,enum=rust2rpm,title=Origin type,description=Type of origin for this source file"`
	...
	// CratesFile identifies the .crate file to use for the rust2rpm origin type.
	// Relative to the sources directory. Ignored for other origin types.
	CratesFile string `toml:"crates-file,omitempty" json:"cratesFile,omitempty" jsonschema:"title=Crates file,description=Path to the .crate file relative to the sources directory (for rust2rpm origins)"`
}

// IsGenerative returns true if this origin produces files at runtime rather than
// downloading them from a cache or URI. Generative origins (e.g., rust2rpm) do not
// have lookaside hashes and their hashes are computed from the generated files.
func (o Origin) IsGenerative() bool {
	switch o.Type {
...
// RustVendorConfig encapsulates configuration for Rust vendor bundling.
// When enabled, azldev synthesizes source-file entries to generate a vendored
// dependencies tarball (via cargo) and a vendor-aware spec (via rust2rpm).
type RustVendorConfig struct {
	// Enabled indicates whether Rust vendor bundling is active for this component.
	Enabled bool `toml:"enabled" json:"enabled" jsonschema:"title=Enabled,description=When true enables Rust vendor bundling for this component"`
	// CratesFile optionally specifies the .crates file name relative to the sources directory.
	// When omitted, azldev auto-discovers the .crates file by globbing *.crates in the sources directory.
	CratesFile string `toml:"crates-file,omitempty" json:"cratesFile,omitempty" jsonschema:"title=Crates file,description=Name of the .crates file relative to the sources directory. Auto-discovered when omitted."`
}
...
type ComponentConfig struct {
	...
	// Rust vendor bundling configuration for this component.
	RustVendor RustVendorConfig `toml:"rust-vendor,omitempty" json:"rustVendor,omitempty" table:"-" jsonschema:"title=Rust vendor,description=Configuration for Rust vendor bundling"`
}
...
func (c *ComponentConfig) WithAbsolutePaths(referenceDir string) *ComponentConfig {
	...
	result := &ComponentConfig{
		...
		RustVendor:       deep.MustCopy(c.RustVendor),
	}
