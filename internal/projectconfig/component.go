...
const (
	...
	// OriginTypeCargoVendored indicates that the source file is a vendor tarball
	// generated by running 'cargo vendor' against the component's Cargo.toml.
	OriginTypeCargoVendored OriginType = "cargo-vendored"
)

// IsGenerated reports whether the origin type produces files at build time
// (e.g., by running a tool) rather than downloading a pre-existing artifact.
// Generated files cannot have a pre-computed hash in the configuration;
// the hash is always computed from the produced file.
func (t OriginType) IsGenerated() bool {
	switch t {
...
type Origin struct {
	...
	Type OriginType `toml:"type" json:"type" jsonschema:"required,enum=download,enum=cargo-vendored,title=Origin type,description=Type of origin for this source file"`
	...
	// Source names the archive file (e.g., a '.crate' or source tarball) in the
	// same directory that contains the manifest needed by this origin handler.
	// Required when type is 'cargo-vendored': the handler extracts this archive
	// to locate 'Cargo.toml' before running 'cargo vendor'.
	Source string `toml:"source,omitempty" json:"source,omitempty" jsonschema:"title=Source archive,description=Filename of the source archive containing the manifest (e.g. Cargo.toml) used by this origin handler. Required for 'cargo-vendored'.,example=my-crate-1.0.crate"`
	// Patches lists patch files to apply to the extracted source before running
	// the origin handler (e.g., patching 'Cargo.toml' before 'cargo vendor').
	Patches []string `toml:"patches,omitempty" json:"patches,omitempty" jsonschema:"title=Pre-handler patches,description=Patch files to apply to the extracted source before running the origin handler (e.g. patches that modify Cargo.toml/Cargo.lock).,example=relax-deps.patch"`
}
