...
func TestPrepareSources_SourceManagerError(t *testing.T) {
	...
	component.EXPECT().GetName().AnyTimes().Return("test-component")
	sourceManager.EXPECT().FetchComponent(gomock.Any(), component, testOutputDir, gomock.Any()).Return(expectedErr)
...
func TestDiffSources_FetchError(t *testing.T) {
	...
	sourceManager.EXPECT().FetchComponent(gomock.Any(), component, gomock.Any(), gomock.Any()).Return(expectedErr)
